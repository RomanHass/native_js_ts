import { CityType } from "../02/hw_02";
import { getStreetsTitlesOfGovernmentBuildings, getStreetsTitlesOfHouses, greetengsMessageForStreets } from "./HW_05";
...
test('create greeting messages for streets', () => {
  ...
  expect(getGreetengsMessages.length).toBe(3);
  expect(getGreetengsMessages[0]).toBe('Hello White street!');
  expect(getGreetengsMessages[1]).toBe('Hello Happy street!');
  expect(getGreetengsMessages[2]).toBe('Hello Happy street!');
});